...
response = completion(hello_message, tools)
response_with_tool = completion(tool_message, tools)
response_with_multiple_tools = completion(multiple_tool_message, tools)

print("單純聊天的回覆",response.message)
print("偵測到需要工具的回覆", response_with_tool.message)
print("偵測到需要多個工具的回覆", response_with_multiple_tools.message)